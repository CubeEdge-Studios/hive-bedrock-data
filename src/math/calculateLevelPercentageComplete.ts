...
export default function calculateLevelPercentageComplete(xp: number, game_id: Game): number {
    ...
    const current_level_xp = calculateXPFromLevel(Math.floor(current_level), game_id);
    if (current_level_xp === null) return 0;

    const level_xp = xp - current_level_xp;

    const total_level_xp = calculateLevelXP(Math.floor(current_level + 1), game_id);
